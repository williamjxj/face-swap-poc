...
export async function POST(request) {
  try {
    ...
    const videoPathWithoutLeadingSlash = videoPath.startsWith('/') ? videoPath.substring(1) : videoPath;
    ...
    const relativeThumbnailPath = result.thumbnailPath ? 
      result.thumbnailPath.replace(publicDir, '') : null;
    ...
    return NextResponse.json({
      ...
      optimized: {
        ...
        thumbnailPath: relativeThumbnailPath ? 
          (relativeThumbnailPath.startsWith('/') ? relativeThumbnailPath : `/${relativeThumbnailPath}`) : null,
        ...optimizedInfo
      },
      stats: {
