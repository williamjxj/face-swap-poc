 ...
 */
export async function addTextWatermark(inputPath, {
  outputPath = null,
  ...
  try {
    ...
    const ffmpegArgs = [
      '-i', inputPath,
      '-vf', `drawtext=text='${text}':fontcolor=${fontColor}@${opacity}:fontsize=${fontSize}:x=${positionCoordinates.split(':')[0]}:y=${positionCoordinates.split(':')[1]}`,
 ...
 * @returns {Promise<Object>} - Path to the watermarked video
 */
export async function addImageWatermark(inputPath, watermarkImagePath, {
  outputPath = null,
  position = 'bottomright',
  scale = 0.2,
  opacity = 0.7
  ...
  try {
    ...
    const ffmpegArgs = [
      ...
      '-filter_complex', `[1:v]scale=iw*${scale}:-1,format=rgba,colorchannelmixer=aa=${opacity}[overlay];[0:v][overlay]overlay=${positionCoordinates}:format=auto,format=yuv420p`,
      '-codec:a', 'copy', // Copy audio stream without re-encoding
      '-y', // Overwrite output file if exists
      outputPath
    ];
